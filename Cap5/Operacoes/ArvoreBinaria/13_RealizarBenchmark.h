...
void realizarBenchmark(pDArvore arvore)
{
    ...
    for (i = 0; i < NUM_OPERACOES; i++)
    {
        ...
        BuscarInfo(arvore, AlocaInt(numeros_inseridos[indice_aleatorio]), ComparaInt);
    }
    fim = clock();
    ...
    {
        int indice_aleatorio = rand() % NUM_ELEMENTOS;
        ExcluirInfo(arvore, AlocaInt(numeros_inseridos[indice_aleatorio]), ComparaInt);
    }
    fim = clock();
